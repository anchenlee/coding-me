        ...
        if (ISWP){
            ...
            if (profile){
                profile.setAttribute("href", "weixin://profile/gh_0d06601f940f");
            }
        }
        var tid = "";
        var aid = "";
        ...
        var title     = trim('绑定/解绑QQ号/手机号/邮箱'      .replace(/&lt;/g, '<').replace(/&gt;/g, '>'));
        ...
        function report_article(){
            ...
            location.href = '/mp/readtemplate?t=wxm-appmsg-inform&__biz=MzA5MDAwOTExMw==&info=' + encodeURIComponent(query) + "#wechat_redirect";
        }
        ...
        (function(){
        function onBridgeReady(imgg,linkk,titlee,descc) {
                desc   = desc || link;  
            if( "1" == "0" ){
        ...
        (function(){
            var timeout = null;
            var val = 0;
            var url = "http://mp.weixin.qq.com/mp/appmsg/show?__biz=MzA5MDAwOTExMw==&appmsgid=10000008&itemidx=1&sign=12291b2b851665eca92f4d2c2a14e408#wechat_redirect".split('?').pop();
            var key = hash(url);
            /*
			...
			for(var i=0, l=imageEls.length; i <l; i++) {
				...
				for(var j = 0; j < entities.length; j++) {
					...
					if (entity.name == src) {
						ajax({	
							type : "POST",
							url: "/mp/appmsgpicreport?__biz=MzA5MDAwOTExMw==" + "&uin=" + uin + "&key=" + key + "#wechat_redirect",
							data: obj2query({
								rnd: Math.random(),
								uin: uin,
