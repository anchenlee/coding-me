...
<%
    ...
    String url = "http://d.b17.cn/sactivity/fh/?u=" + uid64;
    String shareStr = "{\"isshare\":true,\"title\":\"1月集分宝红包发放啦！\",\"content\":\"随手君发放1月购物额外奖励红包啦！等级越高分红越多！点我省钱——淘宝购物就用【随手优惠】\",\"clickurl\":\""+ url +"\",\"imgurl\":\"http://bcs.duapp.com/taoyouhui-ad/adImg/4ns4f9rlvr6.jpg\",\"activity_key\":\"08btqisk\",\"channel\":\"weixin,weibo,pengyou,txwb,qqkj,renren,duoban,email,sm\"}";
    
    String newShareStr = Base64.encode(shareStr.getBytes("UTF-8"));
    
    String cUrl = "suishou://youhui.cn?action_title="+ URLEncoder.encode("1月集分宝红包发放啦", "UTF-8")+"&action_value=" + URLEncoder.encode(newShareStr, "UTF-8")+"&action_type=youhuishare&jump_from=suishou";
    
    Fenhong fh = FenhongDAO.getInstance().getFenhong(uid, "2014", "01");
%>
...
<img src="images/bg1_copy.png" id="bg_pic" alt="" />
		<div id="bg">
			...
			<div class="title"><span class="yellow">本月获得<span class="white"><%=fh.getFhJfbNum()%></span>个集分宝红包</span></div>
			<% }else{%>
		...
		<div id="bill">
			<div class="bill">
			    <table class="tb">
					<tr>
					<td style="width:32%">当月购物奖励<br/><span class="yellow"><%=fh.getJfbNum()%></span><span class="jfb">(集分宝)</span></td>
					<td style="width:32%">
