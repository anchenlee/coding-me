...
<head>
    ...
    <script type="text/javascript">
	    ...
	    function checkParam()
		{
			...
			$.ajax({
    		    url: '<%=request.getContextPath()%>/ad/gift_manager?actionmethod=updateGift',
    		    type: 'POST',
		...
		<jsp:include page="../include/menu.jsp"></jsp:include>
         <div id="primary" class="hfeed">
		   ...
		   <div id="post-8" class="post-8 page type-page status-publish hentry">
	         <h2 class="entry-title">编辑礼物</h2>
				<div id="submit-content">
                <div id="formbox">
                		<input type="hidden" id="gift_id" name="gift_id" value="${GiftBean.id}" />
                		<input type="hidden" id="rank" name="rank" value="${GiftBean.rank}" />
                		<input type="hidden" id="status" name="status" value="${GiftBean.status}" />
                		<p>
                             <label><font color="#777">标题：</font></label>
                             <input type="text" name="title" id="title" value="${GiftBean.title}" />
                        </p>
                        <p>
                             <label><font color="#777">详细内容：</font></label>
                             <input type="text" name="description" id="description" value="${GiftBean.description}" />
                       	</p>
                       	<p>
							<label><font color="#777">按钮名称：</font></label>
							<input type="text" name="clickname" id="clickname" value="${GiftBean.clickname}" />
						</p>
						<p>
							<label><font color="#777">开始时间：</font></label>
							<input type="text" class="ui_timepicker" name="start_time" id="start_time" value="${GiftBean.start_time}" />&nbsp;&nbsp;&nbsp;<font color="red">格式为：2013-11-9 9:23:24</font>
						</p>
						<p>
							<label><font color="#777">结束时间：</font></label>
							<input type="text" class="ui_timepicker" name="end_time" id="end_time" value="${GiftBean.end_time}" />&nbsp;&nbsp;&nbsp;<font color="red">格式为：2013-11-9 9:23:24</font>
						</p>
						<p>
							...
							<select id="action_type" name="action_type" onchange="actionTypeChange_1()">
								<c:forEach var="ActionType" items="${ActionTypeMap}">
									<c:choose>
										<c:when test="${GiftBean.actionType == ActionType.key}">
						...
						<p>
							...
							<input type="text" name="action_value" id="action_value" value="${GiftBean.actionValue}" />
							<select id="xitongyemian" name="xitongyemian"  style="display: none;">
								<c:forEach var="XTYM" items="${XTYM_Map}">
									<c:choose>
										<c:when test="${GiftBean.actionValue == XTYM.key}">
							...
							<select id="tagactionvalue" name="tagactionvalue"  style="display: none;">
								<c:forEach var="tag_map" items="${Tag_Type_Value_Map}">
									<c:forEach var="tag" items="${tag_map}">
										<c:choose>
											<c:when test="${GiftBean.actionValue == tag.key}">
						...
						<p>
							...
							<input type="text" name="img" id="img"  value="${GiftBean.img}" />&nbsp;&nbsp;&nbsp;<font color="red">尺寸：616*268  50KB</font>
						...
						<p>
							...
							<input type="text" name="imgIpad" id="imgIpad"  value="${GiftBean.imgIpad}" />&nbsp;&nbsp;&nbsp;<font color="red">尺寸：306*180 30KB</font>
							<!-- <img id="img_show" src="${activitybean.img}"  height="100"/> -->
						</p>
						<%
	...
	showActionValue('${GiftBean.actionType}');
