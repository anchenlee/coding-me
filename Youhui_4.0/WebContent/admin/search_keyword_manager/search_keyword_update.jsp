...
<head>
    ...
    <script type="text/javascript">
	    ...
	    function checkParam()
		{
			...
			$.ajax({
    		    url: '<%=request.getContextPath()%>/ad/search_keyword_manager?actionmethod=updateSearchKeyword',
    		    type: 'POST',
    		    data: {
    		    	id:id,
    		    	name:name,
		...
		<jsp:include page="../include/menu.jsp"></jsp:include>
         <div id="primary" class="hfeed">
		   ...
		   <div id="post-8" class="post-8 page type-page status-publish hentry">
	         <h2 class="entry-title">编辑搜索关键字</h2>
				<div id="submit-content">
                <div id="formbox">
                		<input type="hidden" id="id" name="id" value="${SearchkeywordBean.id}" />
                		<p>
                             <label><font color="#777">关键字：</font></label>
                             <input type="text" name="name" id="name" value="${SearchkeywordBean.name}" />
                        </p>
						<p>
							...
							<select id="action_type" name="action_type" onchange="actionTypeChange_1()">
								<c:forEach var="ActionType" items="${ActionTypeMap}">
									<c:choose>
										<c:when test="${SearchkeywordBean.action_type == ActionType.key}">
						...
						<p>
							...
							<input type="text" name="action_value" id="action_value" value="${SearchkeywordBean.action_value}" />
							<select id="xitongyemian" name="xitongyemian"  style="display: none;">
								<c:forEach var="XTYM" items="${XTYM_Map}">
									<c:choose>
										<c:when test="${SearchkeywordBean.action_value == XTYM.key}">
							...
							<select id="tagactionvalue" name="tagactionvalue"  style="display: none;">
								<c:forEach var="tag_map" items="${Tag_Type_Value_Map}">
									<c:forEach var="tag" items="${tag_map}">
										<c:choose>
											<c:when test="${SearchkeywordBean.action_value == tag.key}">
						...
						<p>
							...
							<input type="text" name="img" id="img"  value="${SearchkeywordBean.icon}" />&nbsp;&nbsp;&nbsp;<font color="red">尺寸：X*X</font>
						</p>
						<%
	...
	showActionValue('${SearchkeywordBean.action_type}');
