...
<script type="text/javascript" src="<%=request.getContextPath()%>/js/custom_blue.js"></script>
    <script type="text/javascript">
		...
		{
	    	var repeated = $("#deny_repeated").val();
	    	if(repeated != 'yes'){
	    		return;
	    	}
	    	$("#deny_repeated").val("no");
	    ...
	    function checkClickNum(url){
    		if(url == ''){
    			alert("链接地址不能为空！");
    			return;
    		}
    		var key = url.replace("http://b17.cn/","").replace(/\s+/g,"");    
    		window.open("<%=request.getContextPath()%>/ad/statistic?actionmethod=checkClickNum&title=商品&id="+key);   		
    	}
    </script>
	</head>
			...
			<table class="tablewidget" width="100%"> 
	    		<thead class="title">
	    			<tr>
	    				...
	    				<c:if test="${tag_id==1249 || parent==1249}">
	    				<th style="width: 8%">查看点击量</th>
	    				</c:if>
	    			</tr>
	    		</thead>
	    		<tbody id="tablebd">
	    			<c:forEach var="item" items="${ItemList}"  varStatus="status">
	                 <tr id="tr${item.item_id}">
	                 	...
	                 	<c:choose>
                       	...
                       	<td width="150px;">
                       		<a target="_blank" href="<%=request.getContextPath() %>/admin/tag_manager/discount_product_taobao.jsp?itemid=${item.item_id}" style="color: #1F92FF">
                       			 <span id="itemtitle${item.item_id}">
                       			  ${item.title}
                       			 </span>
                       			</a>
                       		</td>
                       	...
                       	<td>
						    ...
						    <input type="hidden" name="fileName" value="<%=imgSaveName%>">
	                         ...
	                         <iframe name="uploadIfr" id="uploadIfr"  style="display: none" onload="displayResult(this)"></iframe>
	                       	 ...
	                       	 <c:if test="${tag_id==1249 || parent==1249}">
	    				<td style="width: 8%">
	    				<a href="javascript:checkClickNum('${item.clickURL}')">
	    					查看点击量
	    				</a>
	    				</td>
	    				</c:if>
