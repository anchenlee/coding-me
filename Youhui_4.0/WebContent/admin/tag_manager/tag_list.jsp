...
<head>
    ...
    <script type="text/javascript">
	    ...
	    function goto_fixed_tag(){
	    	location.href="<%=request.getContextPath()%>/ad/tag_manager?actionmethod=getFixedTag";
	    }
	    function changeStatus(obj){
	    	$.ajax({url: "<%=request.getContextPath()%>/ad/tag_manager?actionmethod=dealTagStatus",
			    type: "POST",
				data: '&status=' + obj,
		...
		<jsp:include page="../include/menu.jsp"></jsp:include>
         <div id="primary" class="hfeed">
	         ...
	         <form action="<%=request.getContextPath()%>/ad/tag_manager?actionmethod=showTagListToAdmin&parent=${parent}&status=${status}&ispad=${ispad}" method="post" >
		         ...
		         <input type="button" class="button" value="新增标签" onclick="goto_add_tag()" />
		         <input type="button" class="button" value="PAD标签管理" onclick="goto_pad_tag()" />
		         <input type="button" class="button" value="热门商品" onclick="goto_hot_item()" />
		         <input type="button" class="button" value="混合样式" onclick="goto_mix_list()" />
		         <input type="button" class="button" value="固定标签" onclick="goto_fixed_tag()" />
                 
                  	<strong>返集分宝开关：</strong>
    			 <select class="sel" onchange="changeStatus(this.value)">
    			 <c:choose>
		          	<c:when test="${tagStatus == 1}">
		    ...
		    <hr/>
		     <c:if test="${chaojiHuiList != null || fn:length(chaojiHuiList) > 0}">
			    <c:forEach var="tag_map_2" items="${chaojiHuiList}">
		    ...
		    <hr/>
		    <c:if test="${yuList != null && fn:length(yuList) > 0}">
			    <c:forEach var="tag_map_2" items="${yuList}">
				    <c:forEach var="tag_2" items="${tag_map_2}">
						<a href="<%=request.getContextPath()%>/ad/tag_item_manager?actionmethod=showTagItemList&parent=${parent}&tag_id=${tag_2.key}&tag_name=${tag_2.value}" style="color: #1F92FF">${tag_2.value}</a>&nbsp;|&nbsp;
					</c:forEach>
				</c:forEach>
	    			...
	    			<c:forEach var="tag" items="${tagList}">
	                 <tr>
								...
								<c:when test="${ispad == 0}">
									<label class="media_photos">
				                       	<a rel="slide" target="_blank" href="${tag.phone_icon}" title="${tag.tag_name}">
				                       		<img width="100px"  src="${tag.phone_icon}">
				                       	</a>
				                    </label>
								 </c:when>
								 <c:otherwise>
								 	<label class="media_photos">
				                       	<a rel="slide" target="_blank" href="${tag.pad_icon}" title="${tag.tag_name}">
				                       		<img width="100px"  src="${tag.pad_icon}">
				                       	</a>
				                    </label>
								 </c:otherwise>
							</c:choose>
						...
						<a onclick="movePosition(${tag.tag_id},${tag.rank},4)">
	                   	...
	                   	<td>
							<a href="<%=request.getContextPath()%>/ad/tag_manager?actionmethod=showUpdateTagJSP&page=${page}&parent=${parent}&status=${status}&ispad=${ispad}&tag_id=${tag.tag_id}">编辑</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                        	<a href="<%=request.getContextPath()%>/ad/tag_manager?actionmethod=showTagListToAdmin&parent=${tag.tag_id}&ispad=0">子标签管理</a>&nbsp;&nbsp;|&nbsp;&nbsp;
