...
public class ProfRecom {
	 ...
	 * 折扣价格
	 */
	private double itemPromPrice = 0;
	...
	public String toXML(){
		String img=Util.getSimpleImg(itemImg, "400x400");
		if(itemImg.indexOf("fixedtag")>0){
			img=itemImg.split("fixedtag")[0];
		}
		StringBuffer sb = new StringBuffer();
		String imgWidth = "0";
		String imgHeight = "0";
		if(!StringUtils.isEmpty(imgSize) && imgSize.indexOf("x") >-1){
			imgWidth = imgSize.substring(0, imgSize.indexOf("x"));
			imgHeight = imgSize.substring(imgSize.indexOf("x")+1, imgSize.length());			
		}
		sb.append("<prof_recom>")
		.append("<is_item>").append(isItem).append("</is_item>")
		.append("<id>").append(id).append("</id>")
		.append("<fav_num>").append(favNum).append("</fav_num>")
		.append("<img width='"+ imgWidth +"' height='" + imgHeight + "' ><![CDATA[").append(img).append("]]></img>")
		...
		sb.append("<orgin_price>").append(itemPrice).append("</orgin_price>");
		sb.append("<is_baoyou>").append(baoyou).append("</is_baoyou>");
		String discountStr = "10.00";
		try {
			if(itemPromPrice != itemPrice && (itemPromPrice/itemPrice) != 1)
			 discountStr = df.format(itemPromPrice*10/itemPrice);
		} catch (Exception e) {
			discountStr = "10.00";
		}
		sb.append("<discount>").append(discountStr).append("</discount>");
		sb.append("</item>");
	...
	public String toJson(){
		...
		sb =  new StringBuffer().append("{\"prof_recom\":")
				.append("{\"is_item\":\"").append(isItem).append("\",")
				.append("\"id\":\"").append(id).append("\",")
				.append("\"fav_num\":\"").append(favNum).append("\",")
				.append("\"img\":{\"img\":\"").append(itemImg).append("\"").append(",\"width\":").append(imgWidth).append(",\"height\":").append(imgHeight).append("},")
				.append("\"recommend_reason\":\"").append(recommendReaSon).append("\",");
				if(action != null){
				...
				.append("\"is_baoyou\":\"").append(baoyou).append("\",")
				.append("\"orgin_price\":\"").append(itemPrice).append("\",")
				.append("\"discount\":\"").append(discountStr).append("\",")
				.append("\"item_id\":\"").append(itemId).append("\",")
				.append("\"jfb_rate\":\"").append(2.0).append("\",")
				.append("\"title\":\"").append(itemTitle.replaceAll("<[^>]+>", "")).append("\",")
				.append("\"price\":\"").append(itemPromPrice).append("\"")
				.append("}}}");
		return sb.toString();
    }
