...
public class TuiGuangThread implements Runnable{
	...
	public TuiGuangThread(ParamBean bean,long activatetime,String uid,int from) {
		this.bean = bean;
	...
	public void run() {
		try{
			...
			//奇葩情况，两个叠加在一起：114.253.210.114, 223.202.80.24
	...
	//激活
	public void activate(String device_token,String idfa,long activatetime,String adalias,String activateip,String code,String openudid){
		//获取设备ID
	...
	//注册
	public void register(String device_token,String idfa,long activatetime,String adalias,String activateip,String code,String openudid,String uid){
		//获取设备ID
		String did = TuiGuangDao.getInstance().getDid(device_token);
		if(did != null && !"".equals(did)){
			//获取推广商ID
	...
	//签到
	public void sign(String device_token,String idfa,long activatetime,String adalias,String activateip,String code,String openudid,String uid){
		//获取设备ID
		String did = TuiGuangDao.getInstance().getDid(device_token);
		if(did != null && !"".equals(did)){
			//获取推广商ID
			...
			if(!TuiGuangDao.getInstance().isSignExist(did, spid)){
				//保存签到信息
				TuiGuangDao.getInstance().saveSignInfo(did, spid, activatetime,uid,device_token);
				//更新总表时间
	...
	public void check(String did,String device_token,long activatetime){
	...
	//激活时判断是否超时
	public boolean isOntime(String did,long activatetime) {
		long createteTime = TuiGuangDao.getInstance().getCreateTime(did);
		if((activatetime >= createteTime) && (activatetime-createteTime <= 86400000*3)) {
			return true;
		} else {
			return false;
		}
	...
	//拿不到激活数据时，判断是否超时
	public boolean isOntime2(String did,long activatetime) {
		long updateTime = TuiGuangDao.getInstance().getUpdateTime(did);
		if((activatetime >= updateTime) && (activatetime-updateTime <= 86400000*3)) {
			return true;
		} else {
			return false;
		}
	...
	//保存回调信息
	public void saveInfo(boolean isSuccess,String did,String spid,String device_token,long time){
		if(isSuccess){
	...
	//要不要扣除回调
	public void isCallback(String device_token,String idfa,long activatetime,String adalias,String activateip,String code,String openudid,String did,String spid){
	...
	//回调
	public void doCallback(String did,String spid,String device_token,String idfa,long activatetime,String adalias,String activateip,String code,String openudid){
		boolean isSuccess = false;
		//安沃
		if(spid.equals("1000")){
			isSuccess = new AdWoCallback().callback(idfa, activatetime, adalias, activateip, code, openudid);
			saveInfo(isSuccess,did,spid,device_token,System.currentTimeMillis());
		}
		//米迪
		if(spid.equals("1001")){
