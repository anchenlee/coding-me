...
@WebServlet("/youhuiWebLogin/register")
public class Register extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try{	
			...
			paramMap.put("random", new String[]{randomNum});
			paramMap.put("t", new String[]{time});			
			
			String sign = MD5.digest(Config.SIGN_KEY + StringUtil.paramsToStr(paramMap) + Config.SIGN_KEY);	
			
			String ret = NetManager.getInstance().send(Config.API_V5_FOR_WEB + "/login/register", "platform=PC&phone_num="+URLEncoder.encode(phoneNum,"utf-8")+"&password="+URLEncoder.encode(password,"utf-8")+"&random="+randomNum+"&t="+time+"&sign="+sign); 
